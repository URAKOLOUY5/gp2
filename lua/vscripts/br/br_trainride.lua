...
function PlayShakeRattleSound()
    ...
    if m_nShakeRattleCount == 1 then
        nSoundNum = 2
    elseif m_nShakeRattleCount == 2 then
        nSoundNum = 3
    elseif m_nShakeRattleCount == 3 then
        nSoundNum = 4
    elseif m_nShakeRattleCount > 3 then
        nSoundNum = math.random(1, 4)
    end

    EntFire("rattles_0" .. nSoundNum, "PlaySound", "", 0.0)

    m_nShakeRattleCount = m_nShakeRattleCount + 1
end
...
function ShowEndSteamURL()
    -- Site is dead so show only message :/
    ScriptShowHudMessageAll( "www.super8-movie.com (dead)", 10 );
    EntFire( "command", "command", "disconnect", 12 );
end
